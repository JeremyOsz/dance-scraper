...
function isInLondonEvent(text: string): boolean {
  return /(london|camden|shoreditch|southwark|ealing|enfield|king's cross|dalston|finchley|n\d|e\d|se\d|sw\d|w\d|nw\d)/i.test(text);
}

function isRuedaOrSalsaEvent(text: string): boolean {
  return /(salsa|rueda|casino|cuban|timba|son cubano|latin kiss|ritmo londres|cubaneando)/i.test(text);
}
...
export async function scrapeSalsaRuedaRuedaLibre(): Promise<AdapterOutput> {
  try {
    ...
    for (const block of blocks) {
      ...
      if (!isRuedaOrSalsaEvent(sourceText)) continue;
      if (!isInLondonEvent(sourceText)) continue;
